# Resume: Logistics & Supply Chain / Project Manager
**Best for:** Logistics PM, supply chain, operations, warehouse management roles

---

**ALEXANDER TOBIN**
(757) 289-1204 | atobin@alum.utk.edu | Virginia Beach, VA

## Professional Summary
Seasoned Data Analyst experienced with project management. Very versed in Logistics & Supply Chain. Strong ability to identify specific insights through data manipulation and analysis (excel functions, cleaning, sorting & filtering etc.). Experienced with all aspects of internal business operations as an analyst and a PM. Very quick learner with a keen attention to detail. Always motivated to go above and beyond for the team.

## Skills
- Freight Documentation (BOL, Delivery Receipts, Certificates, etc.)
- Microsoft Excel (VLOOKUP, Nested IF, Index, Keyboard Shortcuts, etc.)
- Organization (Outlook optimization)
- Pivot Charts
- Problem-solving
- Verbal and written communication
- Shipping and receiving
- Warehouse operations
- Team management
- Task prioritization

## Relevant Software
- Microsoft Excel, Word, PowerPoint & Outlook
- Microsoft SQL Server Management Studio
- Microsoft Power BI
- Microsoft Project
- Atlassian Project Management (JIRA)
- Airtable Project Management
- Tableau
- RStudio
- Google Sheets

## Education
**Bachelor of Arts** — Pre Law/Legal Studies, Business Concentration
University of Tennessee, Knoxville, TN — Aug 2019

## Certifications
- Google Analytics Certification
- Google Project Management Certification
- UVA Project Management Course
- CrossFit Level 2 Trainer Certification

## Professional Experience

### Freelance Developer & Logistics Analyst Contractor | Self-Employed | Jan 2026 – Present
- Built **Container Tracker** — full-stack freight container tracking platform (React, Supabase, Vercel) with live shipping API integration, a 6-stage status pipeline (Origin → In Transit → Destination → Customs → Drayage → Delivered), and a fleet-wide dashboard for real-time visibility.
- Delivered logistics data tools and operational reporting solutions for contract clients.

### Supply Chain Analyst | Born Primitive, Virginia Beach, VA | Aug 2024 – Nov 2024
- Monitored carrier performance against SLA requirements and recommended corrective actions.
- Maintained customer scorecards tracking KPIs including OTIF, service levels, and compliance markers.
- Managed inventory levels across multiple distribution stages and helped oversee inbound transport technology enhancements.
- Created and documented SOPs for supply chain management processes.

### Logistics Project Manager | Ferguson Enterprises, Remote | Mar 2022 – Jul 2024
- Managed and directed reporting and tracking of shipments, financial analysis & reporting, and inventory control.
- Coordinated logistics management functions for supplies, equipment, and company assets.
- Maintained and developed shipping database including specifications, descriptions, and dating.
- Managed tax compliance for all orders and obtained supporting documentation from claimants for vendor transfer.

### Warehouse Manager | ZenEvo Chocolate, Knoxville, TN | Jan 2021 – Dec 2021
- Responsible for comprehensive inventory reporting.
- Compiled and sorted information to identify different types of relevant data to improve business performance.
- Facilitated documents including BOLs, Weight Inspection Certificates, NMFC Certificates, Letters of Authority, and Delivery Receipts.
- Led multi-functional team to improve compliance, drive cost control and maintain on-time service delivery.
- Analyzed data to develop and deploy strategies enabling growth, improving costs and driving performance.
- Developed relationships with transportation vendors to reduce cost and improve transport times.

### Project Manager Trainee | Kitchen Tune Up, Knoxville, TN | Feb 2020 – Aug 2020
- Implemented standards, methods, and tools to complete project work and mitigate constraints.
- Coordinated project timelines, meeting agendas, and next steps to move cross-functional projects forward.
- Determined interdependencies to analyze and establish project critical paths.
- Communicated with site supervisors and team members to complete projects on schedule.

### Data Analyst Student Assistant | University of Tennessee, Knoxville, TN | Nov 2014 – Dec 2019
- Compiled and sorted information to prepare source data for computer entry.
- Leveraged available data to create charts and presented findings via written reports and in-person presentations.
- Maintained database files and spreadsheets to prevent data loss.
- Collected and analyzed data from multiple sources.
- Applied analytical tools and methods to inform business decision-making processes.
